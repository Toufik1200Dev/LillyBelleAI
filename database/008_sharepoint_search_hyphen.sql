-- ============================================================
-- 008_sharepoint_search_hyphen.sql
-- Match opportunity refs whether user/query uses spaces or hyphens (2601-d7691 vs 2601 d7691).
  ...
  matched AS (
    SELECT
      ...
      CASE
        ...
        WHEN d.title ILIKE '%' || replace(ts.qq, ' ', '-') || '%' THEN 2e5::double precision
        WHEN replace(d.title, '-', ' ') ILIKE '%' || ts.qq || '%' THEN 2e5::double precision
        WHEN d.folder ILIKE '%' || ts.qq || '%' THEN 8e4::double precision
        WHEN d.folder ILIKE '%' || replace(ts.qq, ' ', '-') || '%' THEN 8e4::double precision
    ...
    WHERE (categories IS NULL OR array_length(categories, 1) IS NULL OR d.category = ANY(categories))
      ...
      AND (
        ...
        OR d.title ILIKE '%' || replace(ts.qq, ' ', '-') || '%'
        OR replace(d.title, '-', ' ') ILIKE '%' || ts.qq || '%'
        OR d.folder ILIKE '%' || ts.qq || '%'
        OR d.folder ILIKE '%' || replace(ts.qq, ' ', '-') || '%'
      )
  )
  SELECT
    matched.title,
